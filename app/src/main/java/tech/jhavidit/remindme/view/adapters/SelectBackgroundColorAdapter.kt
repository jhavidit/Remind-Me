...
class SelectBackgroundColorAdapter(private val clickListen: AdapterInterface) :
    ...
    override fun onBindViewHolder(holder: MyViewHolder, position: Int) {
        ...
        holder.itemView.color_card.setOnClickListener {
            isSelected = position
            clickListen.clickListener(color[isSelected])
            notifyDataSetChanged()
        }

        if (position == isSelected) {
            holder.itemView.color_card.strokeColor =
                ContextCompat.getColor(holder.itemView.context, R.color.purple_200)
            holder.itemView.color_selected.visibility = VISIBLE
        } else {
            holder.itemView.color_card.strokeColor =
                ContextCompat.getColor(holder.itemView.context, R.color.description_color)
